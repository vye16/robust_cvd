...
def jpg2png(i, out_dir, path):
    out_path = os.path.join(out_dir, "frame_{:06d}.png".format(i))
...
def copy_frames(seq):
    ...
    jpg_dir = os.path.join(DAVIS_DIR, seq)
    jpg_paths = sorted(glob.glob("{}/*.jpg".format(jpg_dir)))
    frame_dir = os.path.join(output_dir, "color_full")
    os.makedirs(frame_dir, exist_ok=True)
    for i, path in enumerate(jpg_paths):
        jpg2png(i, frame_dir, path)
    print("copied frames from {} to {}".format(jpg_dir, frame_dir))
...
def visualize_results(seq, fps=10):
    ...
    video.write_videofile(osp.join(output_dir, 'video_comparison.mp4'), fps=fps, codec='mpeg4')
